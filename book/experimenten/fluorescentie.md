...
```
8)  Nu gaan we de data verwerken. Type in de terminal en druk op enter:
```bash
python pulse_dashboard.py 
```
9) Klik op de URL die daar verschijnt. Er wordt een webpagina geopend (heel even geduld). 
10) Upload hierin de .csv .
11) Na even wachten verschijnt het histogram en de curve-fit functie. 
12) De breedte van de 'bins' ( de verticale balken) kun je instellen met de schuifbalk. Probeer maar eens.
13) Rechts kan je de halfwaardetijd aflezen zoals deze berekend is uit de curve-fit. Hieronder een voorbeeld (weer).
